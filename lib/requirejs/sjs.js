...
define(['.requirejs/seriousjs-embed'], function (SeriousJs) {
    'use strict';
    var fs, getXhr,
    ...
    return {
        ...
        load: function (name, parentRequire, load, config) {
            ...
            fetchText(path, function (text) {
                var cfg = {};
                for (var k in (config.SeriousJs || {})) {
                    cfg[k] = config.SeriousJs[k];
                }
                cfg.amdModule = true;
                if (!config.isBuild) {
                    cfg.sourceMap = SeriousJs.sourceMap;
                }
                cfg.filename = path;

                //Do sjs transform.
                try {
                  var compiled = SeriousJs.compile(text, cfg);
                }
                catch (err) {
                ...
                var jsText = compiled.js;
                /** When using a proper sourceMappingURL, don't need sourceURL
                if (!config.isBuild) {
                    jsText += "\n//@ sourceURL=" + path;
                }*/
                ...
                if (config.isBuild) {
                    buildMap[name] = jsText;
                }

                load.fromText(name, jsText);

                //Wait for it to finish parsing, I guess?  Unsure, copied this
                //last bit from the coffeescript one in case it does anything.
